...
public class PedidoDAO {
    ...
    private final String stmtSomarPedidos = "SELECT SUM(custo) AS SumResult FROM pedido WHERE cpf = ?;";
    ...
    public double somarPedidos(int cpf){
        ...
        try {
            connection = new ConnectionFactory().getConnection();
            stmt = connection.prepareStatement(stmtSomarPedidos);
            stmt.setInt(1, cpf);
            rs = stmt.executeQuery();
            while (rs.next()) {
                somaValores = (rs.getDouble("SumResult"));
            }
            return somaValores;
        } catch (SQLException e) {
            throw new RuntimeException(e);
